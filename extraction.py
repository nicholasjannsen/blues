...
def trace(image_data, smooth_win=10, exclude_border=10, min_distance=40, threshold_abs=0, trace_tol=5, \
    ...
    min_distance   (int, float): Minimum distance to locate where the orders are 
    threshold_abs  (int, float): 
    trace_tol      (int, float): 
    num_orders     (int, float): User specified number of orders the program should find
    num_peaks      (int, float): 
    ----------------------------
               OUTPUT          :
    ----------------------------
    order_traces   (dict)      : Dict with orders within 'order x' and corresponding array with polynomials. 
    """
    
    # Image constants:
    center_disp = int(image_data.shape[0] / 2)               # Center position of dispersion axis
    ...
    if plot==1:
        ...
        #plt.plot(np.polyval(poly_coefs,order_trace_disp),order_trace_disp, 'r-')
        plt.show()
...
def make_inter_order_mask(image_data, order_width, order_traces, low_nudge=1., high_nudge=2., plot=0):
    """
    This function looks at the flux level in between the orders ("inter-order") and make and return a mask
    with ones for which is inter-order and zero elsewhere. 
    ----------------------------
                INPUT          :
    ----------------------------
    image_data       (2d array): Master flat 
    order_width    (int, float): Width of inter-order mask
    order_traces         (dict): Traced orders found from the function 'trace'
    ...
    for disp_i in range(disp_length):
        lower_inter_order_edge = np.round(cross_order_position[disp_i] - order_width/2 - low_nudge - \
                                          size_before).astype(int)
        #print(size_before); print(size_after)
        
        if lower_inter_order_edge < 0: lower_inter_order_edge = 0

        #print(disp_i); print(cross_order_position[disp_i])
        #print(lower_inter_order_edge)
    ...
    #                                       size_after)
    #     if upper_inter_order_edge > 0: lower_inter_order_edge = 0
        
    #     print(upper_inter_order_edge)
    #     print(disp_i); print(cross_order_position[disp_i])
    ...
    if plot==1:
        ...
        plt.imshow(image_data, vmin=300, vmax=1e4, alpha=0.5)
        plt.show()
...
def remove_background(image_data, mask, poly_order_x=4, poly_order_y=4, nsteps=1000, medianfiltersize=5, \
    ...
    for yind in yvals:
        ...
        xfitarr[ycount, :] = xfit      # image background
        ycount = ycount + 1
...
def cut_out_order(order_trace, data, cross_order_width, plot):

    half_width = int(cross_order_width/2.)  # Half the width length of the dispersion order
